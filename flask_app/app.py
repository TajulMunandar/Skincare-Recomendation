...
def get_data():
    ...
    laravel_api_url = (
        "http://127.0.0.1:8000/api/get-data"  # Ganti dengan URL yang benar
    )
    ...
    try:
        ...
        skincare_data["Features"] = (
            skincare_data["Id"].astype(str)
            + " "
            + skincare_data["Brand"]
            + " "
            ...
            + " "
            + skincare_data["Kategori"]
            + " "
            + skincare_data["Umur"]
            + " "
            + skincare_data["Harga"].astype(str)
        )
...
def recommend_skincare(
    user_input, skincare_data, tfidf_vectorizer, tfidf_matrix, num_recommendations=3
):
    ...
    if request.method == "POST" and request.form:
        ...
        if skincare_data is not None:
            ...
            recommended_products = recommend_skincare(
                user_input, skincare_data, tfidf_vectorizer, tfidf_matrix
            )
            return (
                jsonify(
                    status="success",
                    message="Rekomendasi berhasil ditemukan",
                    data=recommended_products.to_dict(),
                ),
                200,
            )
        else:
